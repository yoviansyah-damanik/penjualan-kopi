<div>
        ...
        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
            <div class="relative flex-none w-full mt-1 mb-3 lg:w-48 sm:w-64 xl:w-96">
                <input type="text" wire:change="resetPage" wire:model.live='search'
                    ...
                    placeholder="{{ __('Search for :search by :1 or :2', ['search' => __('payment vendors'), '1' => 'id', '2' => __(':name name', ['name' => __('payment vendor')])]) }}">
            ...
            <button id="createPayment VendorButton"
                ...
                type="button" data-drawer-target="drawer-create-payment-vendor-default"
                data-drawer-show="drawer-create-payment-vendor-default"
                aria-controls="drawer-create-payment-vendor-default" data-drawer-placement="right">
                ...
                {{ __('Add new :add', ['add' => __('payment vendor')]) }}
            </button>
        </div>
    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            ...
                            <th scope="col"
                                class="p-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">
                                {{ __(':name Name', ['name' => __('Payment Vendor')]) }}
                            </th>
                            <th scope="col"
                                class="p-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">
                                {{ __('Account Number') }}
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @forelse ($payment_vendors as $payment_vendor)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td>
                                    <div class="flex items-center justify-center w-24 aspect-square">
                                        <img class="w-full" src="{{ $payment_vendor->image_path }}"
                                            alt="{{ $payment_vendor->name }} Image">
                                    </div>
                                </td>
                                ...
                                <td class="p-4 text-base text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ StringHelper::currency($payment_vendor->payments->count()) }}
                                    {{ $payment_vendor->payments->count() <= 1 ? __('payment') : __('payments') }}
                                </td>
                                <td class="p-4 space-x-2 whitespace-nowrap">
                                    <button type="button" id="updatePayment VendorButton"
                                        wire:click="$dispatch('set_payment_vendor_data',{payment_vendor: '{{ $payment_vendor->slug }}'})"
                                        data-drawer-target="drawer-update-payment-vendor-default"
                                        data-drawer-show="drawer-update-payment-vendor-default"
                                        aria-controls="drawer-update-payment-vendor-default"
                                    ...
                                    <button type="button" id="deletePayment VendorButton"
                                        wire:click="$dispatch('set_payment_vendor_delete_data',{payment_vendor: '{{ $payment_vendor->slug }}'})"
                                        data-drawer-target="drawer-delete-payment-vendor-default"
                                        data-drawer-show="drawer-delete-payment-vendor-default"
                                        aria-controls="drawer-delete-payment-vendor-default"
                                        data-drawer-placement="right"
    ...
    {{-- Edit Payment Vendor Drawer --}}
    <div id="drawer-update-payment-vendor-default"
        ...
        <h5 id="drawer-label"
            ...
            {{ __('Update :update', ['update' => __('Payment Vendor')]) }}
        </h5>
        <button type="button" data-drawer-dismiss="drawer-update-payment-vendor-default"
            aria-controls="drawer-update-payment-vendor-default"
        ...
        <livewire:backend.payment-vendor.edit />
    </div>

    {{-- Delete Payment Vendor Drawer --}}
    <div id="drawer-delete-payment-vendor-default"
        class="fixed top-0 right-0 z-40 w-full h-screen max-w-sm p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800"
        tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
        <h5 id="drawer-label"
            class="inline-flex items-center text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">
            {{ __('Delete :delete', ['delete' => __('Payment Vendor')]) }}</h5>
        <button type="button" data-drawer-dismiss="drawer-delete-payment-vendor-default"
            aria-controls="drawer-delete-payment-vendor-default"
            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg">
        ...
        <livewire:backend.payment-vendor.delete />
    </div>

    {{-- Add Payment Vendor Drawer --}}
    <div id="drawer-create-payment-vendor-default"
        class="fixed top-0 right-0 z-40 w-full h-screen max-w-sm p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800"
        tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
        <h5 id="drawer-label"
            class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">
            {{ __('New :new', ['new' => __('Payment Vendor')]) }}
        </h5>
        <button type="button" data-drawer-dismiss="drawer-create-payment-vendor-default"
            aria-controls="drawer-create-payment-vendor-default"
            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd"
        ...
        <livewire:backend.payment-vendor.create />
    </div>
</div>
